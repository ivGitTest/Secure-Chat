...
**[Русская версия](#база-данных--справочник-таблиц)**

ORM: **Drizzle ORM**, DB: **PostgreSQL**.  
Schema: `lib/db/src/schema/`. Migrations applied via `drizzle-kit push`.
...
- [Tables](#tables)
  ...
  - [`participants` — conversation participants (many-to-many)](#participants--conversation-participants-many-to-many)
  - [`messages` — messages](#messages--messages)
  - [`sessions` — active sessions](#sessions--active-sessions)
  - [`devices` — registered devices](#devices--registered-devices)
  - [`push_tokens` — push notification tokens](#push_tokens--push-notification-tokens)
  - [`call_logs` — call logs](#call_logs--call-logs)
- [ER Diagram (text)](#er-diagram-text)
- [Manual Query Connection](#manual-query-connection)
...
`id` set manually on user creation (short readable slug, e.g., `mom`).  
`pin_hash` — Argon2 hash of 4-digit PIN.
...
UPDATE users SET failed_attempts = 0, is_blocked = false WHERE id = 'mom';
```
...
Conversation is just a container. Participants stored in `participants`, messages in `messages`.

```sql
-- All conversations with participant count
SELECT c.id, c.created_at, COUNT(p.user_id) AS participants
FROM conversations c
JOIN participants p ON p.conversation_id = c.id
GROUP BY c.id
...
### `participants` — conversation participants (many-to-many)
...
```sql
-- Conversations for specific user
SELECT c.id AS conversation_id, c.created_at
FROM conversations c
JOIN participants p ON p.conversation_id = c.id
WHERE p.user_id = 'mom';

-- Participants of specific conversation
SELECT u.id, u.name
FROM users u
JOIN participants p ON p.user_id = u.id
WHERE p.conversation_id = '<uuid>';
...
FROM participants p1
JOIN participants p2 ON p2.conversation_id = p1.conversation_id
WHERE p1.user_id = 'mom' AND p2.user_id = 'dad';
```
...
`client_id` — idempotency key generated by client; prevents duplication on retry.
...
One token per user — overwritten on re-registration.
...
SELECT token FROM push_tokens WHERE user_id = 'dad';
```
...
`ended_at` and `duration_s` are NULL for incomplete (dropped) calls.
...
-- Incomplete calls (dropped connections)
SELECT id, caller_id, callee_id, started_at
FROM call_logs
WHERE ended_at IS NULL
  AND started_at < now() - INTERVAL '5 minutes';
...
users (id PK)
 ...
 ├── participants.user_id ──→ conversations (id PK)
 │                               └── participants.conversation_id
 ├── messages.sender_id ──→ conversations.id (via conversation_id)
 ├── call_logs.caller_id
 └── call_logs.callee_id
```
