  ...
  return (
    <div className="c2-root w-[402px] h-[874px] overflow-hidden flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 px-8">
        ...
        {status === 'ok' && (
          <div className="mt-4 c2-card flex items-center gap-2 px-4 py-3">
            ...
            <span className="text-[14px] font-semibold" style={{ color: '#16a34a' }}>Соединение установлено</span>
          </div>
        )}

        {/* Кнопка в нижней трети */}
        <div className="mt-auto pb-10">
          <button type="submit" className="c2-btn" disabled={!url.trim() || status === 'checking'}>
            {status === 'checking' ? (
