...
- [Предварительные требования](#предварительные-требования)
  - [Установка Docker (Ubuntu)](#установка-docker-ubuntu)
...
- [Шаг 3 — Настройка обратного прокси на хосте](#шаг-3--настройка-обратного-прокси-на-хосте)
  - [Если прокси на хосте — nginx (системный сервис)](#если-прокси-на-хосте--nginx-системный-сервис)
  - [Если прокси на хосте — Caddy](#если-прокси-на-хосте--caddy)
  - [TLS-сертификат для прокси на хосте](#tls-сертификат-для-прокси-на-хосте)
...
- [Шаг 5 — Проверка развёртывания](#шаг-5--проверка-развёртывания)
  ...
  - [Установка WebSocket-соединения](#установка-websocket-соединения)
- [Шаг 6 — Настройка push-уведомлений (необязательно, но рекомендуется)](#шаг-6--настройка-push-уведомлений-необязательно-но-рекомендуется)
  - [Обзор](#обзор)
  - [6a — Создание проекта Firebase](#6a--создание-проекта-firebase)
  - [6b — Добавление google-services.json в секреты GitHub Actions](#6b--добавление-google-servicesjson-в-секреты-github-actions)
  - [6c — Добавление google-services.json в секреты EAS (одноразово)](#6c--добавление-google-servicesjson-в-секреты-eas-одноразово)
  - [6d — Подключение Firebase к проекту Expo (одноразово)](#6d--подключение-firebase-к-проекту-expo-одноразово)
...
- [Уведомления об окончании действия сертификата](#уведомления-об-окончании-действия-сертификата)
  ...
  - [Настройка ежедневной задачи cron](#настройка-ежедневной-задачи-cron)
  - [Настройка канала уведомлений](#настройка-канала-уведомлений)
    - [Вариант A — Telegram (рекомендуется, SMTP не нужен)](#вариант-a--telegram-рекомендуется-smtp-не-нужен)
    - [Вариант B — электронная почта](#вариант-b--электронная-почта)
  - [Изменение порога предупреждения](#изменение-порога-предупреждения)
  - [Проверка без ожидания](#проверка-без-ожидания)
- [Автоматическое резервное копирование базы данных](#автоматическое-резервное-копирование-базы-данных)
  - [Ротация резервных копий](#ротация-резервных-копий)
  - [Настройка ежедневной задачи cron](#настройка-ежедневной-задачи-cron)
  - [Необязательно: уведомление об ошибке в Telegram](#необязательно-уведомление-об-ошибке-в-telegram)
  - [Переменные конфигурации](#переменные-конфигурации)
  - [Немедленное создание резервной копии вручную](#немедленное-создание-резервной-копии-вручную)
  - [Проверка создания резервных копий](#проверка-создания-резервных-копий)
  - [Копии за пределами VPS (рекомендуется)](#копии-за-пределами-vps-рекомендуется)
- [Восстановление базы данных](#восстановление-базы-данных)
  - [1 — Выберите файл резервной копии](#1--выберите-файл-резервной-копии)
  - [2 — Остановите API, чтобы новые записи не поступали](#2--остановите-api-чтобы-новые-записи-не-поступали)
  - [3 — Удалите и создайте базу заново](#3--удалите-и-создайте-базу-заново)
  - [4 — Восстановите дамп](#4--восстановите-дамп)
  - [5 — Перезапустите API](#5--перезапустите-api)
  - [6 — Проверьте результат](#6--проверьте-результат)
